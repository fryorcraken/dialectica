...
//!
//! `SPEC.md` assigns sanitisation to the interface, under the heading "Text
//! sanitisation — the interface's job, because the core deliberately does not".
//! Both halves of that heading are true — it *is* the interface that renders,
//! and the core *does* deliberately leave ops unmutated — and they do not imply
//! the conclusion, because §2.1 puts the QML engine in a sandbox with no
//! filesystem and no network: **every byte the view renders arrives through a
//! core method**. A view cannot sanitise text it is handed already rendered,
//! and a view that sanitised in QML would be running attacker-supplied strings
//! through a scripting layer that also holds the rendering primitives.
//!
...
//!
//! `SPEC.md` — the designer's handoff, which is where the interface's
//! obligations arrive from — draws a line this module is built around:
//!
