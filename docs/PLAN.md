...
both are now fixed in that document**, so this block records the *failure mode*
rather than the defects:

- It multiplied the mark against **2²⁵** — a superseded three-word space, stale
  before it merged. It no longer pins the name's space at all; it says to read the
  current figure from this section and multiply by 2¹³·⁶, and carries a worked
  example labelled as possibly stale. **That is the right shape**: a number owned by
  one document should not be duplicated in another.
- It also claimed bytes `0..11` of the **address** were "reserved for the
  generated-name scheme". The name scheme reads no address bytes at all, so nothing
  was reserved and nothing needed to be. Also corrected there.
...
checked. **A word-count change is the same trap in a new costume** — this revision
returns the name to three words, which makes any superseded "three-word" figure
elsewhere look already-correct when it is eight doublings low.
...
**The nouns and the place are Greek; the adjective slot is open.** That split is
...
**Only two screens apply to any list, and there are no others.** This is the
short version of a longer argument that is recorded under "What is excluded"
below, and it is stated here because it is the rule a person writing the lists
...
fantasy handle where *measured aporia of lampsacus* is a name an adult will accept
...
- **~~No single source may dominate.~~ Retired.** It existed because a name
  ...
  nouns and the place have one source, so there is nothing to balance; the
...
**The words that remain excluded are excluded for what they say, not for how
obscure they are** — see the exclusions below, all of which are tone exclusions
and none of which are familiarity exclusions.
...
**The merged design is four words at 2³⁴, with 0.073% at 5,000. This replaces it
...
this section's standard, and it is now the **second** reversal here: an early draft
argued for three words, the merged design overturned it for four, and this returns
to three on a different basis than the first draft had. All three positions are
visible on purpose.
...
of names whose invocation is an argument. This is a structural property of the shape:
pairing a thinker with the place they are conventionally named after signs a user's
posts with a real person's full canonical identifier.
