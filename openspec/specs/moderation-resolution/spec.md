...
The asymmetry is required because a moderation op is fully determined by its Stoa, author, target and action: for one Stoa, one moderator and one target exactly two ops can exist, so an unordered comparison between them resolves the same way permanently rather than until something newer arrives.
