...
**What this change does:** adds two capability specs, a design record, and an
audit of which requirements are pinned by tests.

**What it does not do:** change any code, add any test, or fix any of the four
gaps it found. Every finding is in `design.md` for someone else to act on. A
documents change that also edited the suite could be reviewed as neither.
...
- [x] 5.2 Green after it, over identical source: 126 passed. This change adds no
      Rust, so the gate confirms the worktree was left clean rather than
...
**And one gap in an already-merged spec**, found by this exercise and written up
in `design.md`: `stoa-genesis`'s scenario *A creator key that can never verify a
signature is refused* is pinned by nothing at the genesis boundary. Measured by
disabling the weak-key guard and watching every `stoa.rs` test stay green. The
fix is one test; it is not in this change because this change adds none.
