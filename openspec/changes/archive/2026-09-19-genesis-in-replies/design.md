...
### The record map notifies through an explicit signal

**Chosen:** `screen.genesisByStoa[stoa] = genesis`, followed by an explicit `screen.genesisByStoaChanged()`.

**Why:** a QML `var` property holding a JS object **does not emit a property-changed signal on an in-place key write**. `canShare(row.rowStoa)` is a binding that reads `genesisByStoa`, so without an explicit emit the map would hold the record while the share button stayed hidden and `Open` stayed inert. The data would be right and the screen would be wrong, which is this codebase's house style of silent failure: basecamp swallows QML errors, so nothing would have reported it. The emit is unconditional and present on every path through the function, so it carries this on its own.

**Alternative considered and removed:** an earlier version wrote `var next = screen.genesisByStoa; next[stoa] = genesis; screen.genesisByStoa = next` *as well*, and this entry credited that reassignment for the fix. **Both were wrong.** `next` binds to the same object the property already holds — JS objects are reference types and nothing there cloned — so the "reassignment" assigned the property to the object it already pointed at. Measured with a temporary probe spec (added, run, removed): object identity is unchanged across the call (`before === after`), and the pre-existing reference already sees the newly-written key. The readability review caught the comment; the probe confirmed the mechanism. The line was deleted rather than kept with a corrected note, because a no-op whose only role is to be explained away is the line the next reader mistakes for the mechanism — as this entry itself did.

**What breaks without it — stated honestly, because the answer is "nothing in the suite":** the specs read `canShare` and the map's contents as JS values rather than through a live binding, so **nothing here witnesses the emit at all.** Measured by the readability reviewer, both directions: removing the explicit `genesisByStoaChanged()` left 81/81 green, and removing the reassignment left 81/81 green. This is a known, accepted gap rather than a covered case, and it is wider than this entry previously claimed — it is not "reassignment versus in-place mutation is untested", it is **the notification itself is untested**. Catching it needs a spec that instantiates the row and asserts the button's `visible` after a reload, which is the rendering layer no component test here currently reaches. Named so it is not mistaken for tested; the code comment at `DStoaListScreen.qml`'s `rememberGenesis` points back here.
