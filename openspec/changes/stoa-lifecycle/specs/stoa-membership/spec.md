...
- **AND** it appears without any op having been received for it
...
- **WHEN** creation is attempted with a title longer than a genesis record can carry
- **THEN** the call reports a failure
- **AND** the peer is in no new Stoa afterwards

#### Scenario: A title at the maximum length creates a Stoa

- **WHEN** creation is attempted with a title of exactly the maximum length a genesis record carries
...
### Requirement: Adding membership does not make an existing store unreadable

A store already holding ops MUST remain readable once this peer records Stoa membership alongside it. Opening such a store MUST NOT be refused on the grounds that it predates membership, and its ops MUST still be readable afterwards.
...
### Requirement: An op for a Stoa the peer is not in creates no membership

An op arriving for a Stoa the peer is not in MUST NOT make the peer a member of it. The peer MUST NOT be reported as being in that Stoa, and the Stoa MUST NOT appear in the listing.
...
Whether such an op is stored, and what happens to it, is the `op-log` capability's and the receive path's; this requirement constrains only what it does to membership. The two answers are independent by design — the log may hold ops for Stoas the peer is not in, and that is not a contradiction.

#### Scenario: A gossiped op does not enrol the peer

- **WHEN** an op addressed to a Stoa the peer is not in reaches the peer
- **THEN** the peer is still in no Stoa for that address
- **AND** the listing does not contain it

#### Scenario: Many ops for an unjoined Stoa still enrol nobody

- **WHEN** many ops addressed to one Stoa the peer is not in reach the peer
- **THEN** the peer is still not in that Stoa

#### Scenario: An op for an unjoined Stoa does not disturb the memberships that exist

- **WHEN** a peer in one Stoa receives an op addressed to a different, unjoined Stoa
- **THEN** the peer is still in exactly the one Stoa
- **AND** that Stoa's retained record is unchanged

### Requirement: Membership is not lost because a Stoa has no ops

A Stoa the peer is in MUST remain listed however few ops the peer holds for it, including none at all.
...
#### Scenario: An empty op log does not empty the listing

- **WHEN** a peer is in several Stoas and its op log holds no ops at all
