...
**Reason**: The owner's decision on issue #162 pegs the counter to wall-clock time, as SDS (LIP-109) does. A published op now carries the later of the author's current time and one above its clock, so "one above the highest counter the author has seen" is no longer the rule. The requirement's scenario "A reply orders after the post it replies to" also contradicted the ordering rule, which is issue #162's first item.

**Migration**: Replaced by the ADDED requirement "A published op takes the later of the current time and one above the author's clock". It keeps the causal property, the statement of what a counter does and does not say, and the scenarios "Publishing after receiving advances past what was received" and "Two successive publishes take successive counters". It replaces "A first op in a Stoa carries one" with "A first op in a Stoa carries the current time". It replaces "A reply orders after the post it replies to" with "A reply carries a greater counter than the post it replies to, and the rule places it first". The saturation clause moves into it from the removed advance-bound requirement.

### Requirement: A received counter advances this peer's clock only within a bounded distance

**Reason**: The owner's decision on issue #162 replaces the advance bound with a one-hour receive window. Under the bound, an op carrying a counter far above the clock was stored and placed at the head of the order, while honest clocks never reached it. So an honest peer answering that op signed a lower counter than the op it answered. The requirement also stated, as a deliberate choice, that such an op is not refused. That rule is reversed.

**Migration**: Replaced by the ADDED requirement "An op whose counter is more than one hour ahead of this peer's time is refused on arrival", together with the MODIFIED clock requirement, under which the clock is the highest counter held. "A counter just above this peer's clock advances it" and "A counter within the bound advances the clock" become the clock requirement's "A counter above the clock advances it". "A lower counter never moves the clock backwards" and "Two peers receiving the same ops reach the same clock" move to the clock requirement unchanged. "A maximal counter does not advance the clock" and "A peer can still publish after receiving a maximal counter" become the window's "A maximal counter is refused, and the peer can still publish". "An over-bound op still takes its place in the order" is withdrawn, because such an op is no longer stored. The saturation clause moves to the publish requirement.
